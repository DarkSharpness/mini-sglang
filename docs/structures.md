...
- `minisgl.speculative`: Standalone, opt-in offline speculative decoding (vanilla single-chain, greedy-verify): `SpeculativeEngine` with `verify_drafts` and `StandaloneDraft`, built on HuggingFace transformers and not yet wired into the serving path.
